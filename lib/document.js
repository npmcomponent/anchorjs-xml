...
function(factory, Element, utils) {
  ...
  function Document(ns, name, attrs) {
    ...
    } else if (ns instanceof window.Document) {
      this._root = ns.documentElement;
    } else if (ns instanceof window.Element) {
      this._root = ns;
    }
   ...
   *     el.is('p');
   *
   *     el.is('http://www.w3.org/1999/xhtml', 'div');
   *
   * @return {Boolean}
   * @api public
   */
  Document.prototype.is = function(ns, name) {
    if (!name) {
      name = ns;
      ns = null;
    }
